...
class FlutterFlowDynamicModels<T extends FlutterFlowModel> {
  ...
  final Map<String, T> _childrenModels = <String, T>{};
  final Map<String, int> _childrenIndexes = <String, int>{};
  ...
  List<S> getValues<S>(S? Function(T) getValue) {
    return _childrenIndexes.entries
        // Sort keys by index.
        .sorted((MapEntry<String, int> a, MapEntry<String, int> b) => a.value.compareTo(b.value))
        .where((MapEntry<String, int> e) => _childrenModels[e.key] != null)
        ...
        .map((MapEntry<String, int> e) => getValue(_childrenModels[e.key]!) ?? _getDefaultValue<S>()!)
        .toList();
  }

  S? getValueAtIndex<S>(int index, S? Function(T) getValue) {
    final String? uniqueKey =
        _childrenIndexes.entries.firstWhereOrNull((MapEntry<String, int> e) => e.value == index)?.key;
  ...
  void _updateActiveKeys(String uniqueKey) {
    final bool shouldResetActiveKeys = _activeKeys == null;
    _activeKeys ??= <String>{};
    ...
    if (shouldResetActiveKeys) {
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) {
        _childrenIndexes.removeWhere((String k, _) => !_activeKeys!.contains(k));
        _childrenModels.keys
            ...
            .forEach((String k) => _childrenModels.remove(k)?.dispose());
        _activeKeys = null;
      });
    }
  }
...
extension TextValidationExtensions on String? Function(BuildContext, String?)? {
  String? Function(String?)? asValidator(BuildContext context) =>
      this != null ? (String? val) => this!(context, val) : null;
}
