...
Future launchURL(String url) async {
  final String uri = Uri.parse(url).toString();
  try {
    await launch(uri);
  } catch (e) {
...
dynamic getJsonField(
  ...
  if (field.length > 1) {
    return field.map((JsonPathMatch f) => f.value).toList();
  }
  final Object? value = field.first.value;
  return isForList && value is! Iterable ? <Object?>[value] : value;
}
...
const double kBreakpointSmall = 479.0;
const double kBreakpointMedium = 767.0;
const double kBreakpointLarge = 991.0;
...
const String kTextValidatorUsernameRegex = r'^[a-zA-Z][a-zA-Z0-9_-]{2,16}$';
const String kTextValidatorEmailRegex =
    r"^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,253}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,253}[a-zA-Z0-9])?)*$";
const String kTextValidatorWebsiteRegex =
...
extension IterableExt<T> on Iterable<T> {
  List<S> mapIndexed<S>(S Function(int, T) func) => toList()
      .asMap()
      .map((int index, value) => MapEntry(index, func(index, value)))
      .values
      .toList();
}
/*£
...
extension ListDivideExt<T extends Widget> on Iterable<T> {
  ...
  List<Widget> divide(Widget t) => isEmpty
      ? <Widget>[]
      : (enumerate.map((MapEntry<int, Widget> e) => <Widget>[e.value, t]).expand((List<Widget> i) => i).toList()
        ..removeLast());
  ...
  List<Widget> addToStart(Widget t) =>
      enumerate.map((MapEntry<int, Widget> e) => e.value).toList()..insert(0, t);

  List<Widget> addToEnd(Widget t) =>
      enumerate.map((MapEntry<int, Widget> e) => e.value).toList()..add(t);
}
